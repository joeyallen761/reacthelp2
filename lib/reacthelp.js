...
/*
  ...
  addMarkers(markerType){
    ...
    switch (markerType){
      ...
      markerVars[markerType+"fixAction"] = () => { editor.setText(editor.getText()+"\nexport default "+getFirstComponentName(markerVars[markerType+"editor"].getText())+';');
      ...
      markerVars[markerType+"RegExp"] = RegExp("function|class","s")
      markerVars[markerType+"conditionsFulfilled"] = !(markerVars[markerType+"editor"].getText().includes("ReactDOM") || markerVars[markerType+"editor"].getText().includes("React.render(") || markerVars[markerType+"editor"].getText().includes("export default")) && (markerVars[markerType+"editor"].getText().includes("function") || markerVars[markerType+"editor"].getText().includes("class"))
      markerVars[markerType+"fixAction"] = () => { userEditor.setText(userEditor.getText()+"\nexport default "+getFirstComponentName(markerVars[markerType+"editor"].getText())+';');
...
function h_getFirstComponentName(rawCode){
  ...
  for (i in rawCodeSplit){
    line = rawCodeSplit[i]
    if (line.startsWith("class") || line.startsWith("function ") && line.length>"function ".length){
...
function h_stringReplaceCharWithString(s, c1, c2){
  var to_return = ""
  for (i in s){
    if (s[i] == c1){
      to_return += c2
...
function bindFunctions(rawCode){
    ...
    for (i=0; i<lines_arr.length;i++){
      if ((!lines_arr[i].includes("constructor")) && lines_arr[i].includes("function") && !lines_arr[i].includes("for ") && !lines_arr[i].includes("if ") && !lines_arr[i].includes("else")){
...
function get_HTML_in_topLevel(rawCode){
  ...
  for (i = 0; i < lines.length; i++){
    if (lines[i].trim().startsWith("<")){
      if (rawCode.includes(lines[i])){
        html_lines.push(lines[i])
      }
      else{
        //there must be a function{}
        first_bracket = lines[i].indexOf('{')+1
        left_side = lines[i].substring(0,first_bracket)
        right_side = lines[i].substring(first_bracket)
        left_side = h_stringReplaceCharWithString(left_side,'/','\\/')
        right_side = h_stringReplaceCharWithString(right_side,'/','\\/')
        my_regex = RegExp(left_side+".*"+right_side)
        html_lines.push(rawCode.match(my_regex)[0])
      }
  ...
  for (i = 0; i < html_lines.length; i++){
    html_string += h_stringReplaceCharWithString(html_lines[i], ";","")+"\n"
  }
  ...
  for (i = 0; i < lines.length; i++){
    if (lines[i].trim().startsWith("<")){
      if (rawCode.includes(lines[i])){
        ...
        right_side = h_stringReplaceCharWithString(right_side,'/','\\/')
        my_regex = RegExp(left_side+".*"+right_side)
        rawCode = rawCode.replace(my_regex, '')
      }
    }
  }
  return rawCode
...
function fix_HTML_in_topLevel(rawCode){
  ...
  if (h_getIsFunctional(rawCode)){
    if (rawCode_clean.includes("return")){
      ...
      objectName = h_getFirstExport(rawCode)
      end_bracket_i = h_get_end_index(objectName, rawCode) -1
      //end_bracket_i = rawCode_clean.lastIndexOf("}") -1 //must find end of object }
